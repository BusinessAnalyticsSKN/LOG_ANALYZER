...
%let dir_log=d:\prywatne\log_analyzer_initial\;
%let log=log_analyzer.log;
...
if index(line,'MLOGIC(')>0 and index(line,'Ending execution.')>0 then do; step='E'; code=line; level=level-1; if level=0 then level=1; output; end;
...
if index(line,'MLOGIC(')>0 and index(line,'%DO loop index variable')>0 
	and index(line,'loop will not')>0 then do; loop='E'; code=line; level=level-1; 
...
(real_time cpu_time system_time)*sum=''*f=12.1 
(memory os_memory)*max=''*f=12.1 / printmiss box='Levels';
run;
title 'Levels with codes';
proc tabulate data=Data_log2;
class level1 level2 level3 level4 codes / missing;
var real_time cpu_time system_time memory os_memory;
table 
level1=''*level2=''*level3=''*level4=''*codes='' all,
(real_time cpu_time system_time)*sum=''*f=12.1 
(memory os_memory)*max=''*f=12.1 / printmiss box='Levels';
run;
ods html close;
